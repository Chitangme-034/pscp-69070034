...
```text
The problem wants us to create and input the number of the month the corresponds to each of the 4 seasons(spring, summer, fall, and winter). However, if the date(day) is 21st of every month that could be divided by 3, then the seasons will move upwards.
```
...
```text
Step 1: First is creating 2 int input variables for each input. Month will be the first input and day(for date) will be the second input.
...
```text
To see if the code works or not
```
...
```text
Writing the code and figuring out what the question wants.
```
